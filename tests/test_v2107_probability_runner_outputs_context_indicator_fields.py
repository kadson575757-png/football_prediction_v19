...
def test_probability_runner_outputs_v2107_context_indicator_fields(monkeypatch, tmp_path):
    ...
    for attr, indicator, prefix in [
        ("build_draw_tendency_indicator", "DRAW_TENDENCY", "dt"),
        ("build_venue_result_rate_indicator", "VENUE_RESULT_RATE", "vr"),
        ("build_goal_margin_profile_indicator", "GOAL_MARGIN_PROFILE", "gm"),
        ("build_venue_scoring_balance_indicator", "VENUE_SCORING_BALANCE", "vsb"),
        ...
        ("build_result_streak_indicator", "RESULT_STREAK_PROFILE", "rsp"),
        ("build_scoring_run_indicator", "SCORING_RUN_PROFILE", "srp"),
        ("build_head_to_head_context_indicator", "HEAD_TO_HEAD_CONTEXT_PROFILE", "h2hc"),
        ("build_league_zone_pressure_indicator", "LEAGUE_ZONE_PRESSURE_PROFILE", "lzp"),
    ]:
    ...
    assert result["rsp_adjusted_home_win_probability"] == 0.44
    assert result["srp_adjusted_home_win_probability"] == 0.44
    assert result["h2hc_adjusted_home_win_probability"] == 0.44
    assert result["lzp_adjusted_home_win_probability"] == 0.44
    assert "v2107_mix_adjusted_home_win_probability" in result
    assert "v2107_combined_mix_adjusted_home_win_probability" in result
