...
def test_v296_runner_outputs_goals_for_fields(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "scripts.run_match_winner_analysis.build_goals_for_indicator",
        lambda *args, **kwargs: {
            "goals_for_indicator_quality": "FULL",
            "goals_for_per_match_diff": 0.5,
            "home_goals_for_before_match": 25,
            "away_goals_for_before_match": 18,
            "home_goals_for_per_match_before_match": 2.5,
            "away_goals_for_per_match_before_match": 1.8,
        },
    )
    ...
    assert "gf_adjusted_home_win_probability" in result
    assert "gf_adjusted_draw_probability" in result
    assert "gf_adjusted_away_probability" in result
    ...
    assert result["gf_adjusted_home_win_probability"] != result["base_home_win_probability"]
    assert result["home_goals_for_before_match"] == 25
    assert result["away_goals_for_before_match"] == 18
    assert result["goals_for_per_match_diff"] == 0.5
    assert result["goals_for_indicator_quality"] == "FULL"
