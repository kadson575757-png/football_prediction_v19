...
def test_realdata_smoke_cache_only_repeat(tmp_path):
    ...
    first = run_v20_realdata_smoke_suite(matches=str(matches), output_dir=str(tmp_path / "first"), mock_data_dir=str(mock), cache_dir=str(cache))
    ...
    assert first["matches_total"] == second["matches_total"] == 1
