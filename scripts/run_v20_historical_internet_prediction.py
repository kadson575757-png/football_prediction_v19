...
from football_prediction_v19.analysis.v20_live_source_orchestrator import run_v20_live_source_orchestrator  # noqa: E402
...
def run_v20_historical_internet_prediction(**kwargs: object) -> dict[str, object]:
    ...
    mock_arg = kwargs.get("mock_data_dir")
    mock = _resolve(mock_arg, base) if mock_arg else None
    ...
    live_sources = None
    if mock and (mock / "football_data_matches_mock.csv").exists():
        ...
        live_source_status = "MOCK_MODE"
        cache_used = False
    else:
        live_sources = run_v20_live_source_orchestrator(
            context,
            out,
            ...
            cache_dir=_resolve(kwargs.get("cache_dir") or "outputs/cache/v20_live_sources", base),
            local_fallback_dir=mock,
        )
        football, xg, odds = live_sources["football"], live_sources["xg"], live_sources["odds"]
        live_source_status = str(live_sources.get("live_source_status"))
        cache_used = bool(live_sources.get("cache_used"))
    snapshots = [
    ...
    store["features"]["lineups_available"] = bool(live_sources.get("lineups_available")) if live_sources else False
    store["features"]["injuries_available"] = bool(live_sources.get("injuries_available")) if live_sources else False
    ...
    analyst_report = write_final_historical_analyst_report(context, leakage, merged, store["features"], model, decision, out, live_sources=live_sources)
    artifact_paths = {
    ...
    payload = {"v20_historical_internet_prediction_status": STATUS, "match_context": context.to_dict(), "analysis_cutoff": context.analysis_cutoff, "existing_source_inventory_status": inventory["existing_source_inventory_status"], "live_source_status": live_source_status, "asof_status": merged["asof_status"], "leakage_status": leakage["leakage_status"], "model_status": model["model_status"], "decision_class": decision["decision_class"], "primary_tip": decision["primary_tip"], "confidence": decision["confidence"], "coverage": {k: merged.get(k) for k in ["table_available", "xg_available", "odds_1x2_available", "odds_totals_available"]}, "features": store["features"], "probabilities": {"home": model["home_win_probability"], "draw": model["draw_probability"], "away": model["away_win_probability"]}, "artifact_paths": artifact_paths, "live_sources": live_sources or {}, "network_calls_enabled": bool(kwargs.get("enable_network", False)), "cache_used": cache_used, "automatic_betting_enabled": False, "staking_logic_enabled": False, "roi_logic_enabled": False}
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--cutoff-policy", default="MATCH_DATE_START"); p.add_argument("--mock-data-dir", default=""); p.add_argument("--source-profile", default="config/v20_internet_sources.yaml"); p.add_argument("--output-dir", required=True); p.add_argument("--enable-network", action="store_true"); p.add_argument("--cache-only", action="store_true"); p.add_argument("--cache-dir", default=""); p.add_argument("--emit-all", action="store_true"); p.add_argument("--base-dir", default=str(ROOT))
    args = p.parse_args(argv)
    result = run_v20_historical_internet_prediction(**vars(args))
    result["lineups_available"] = bool(result.get("features", {}).get("lineups_available", False))
    result["injuries_available"] = bool(result.get("features", {}).get("injuries_available", False))
    for key in ["v20_historical_internet_prediction_status", "live_source_status", "asof_status", "leakage_status", "model_status", "decision_class", "primary_tip", "confidence", "table_available", "xg_available", "odds_available", "lineups_available", "injuries_available", "network_calls_enabled", "cache_used", "automatic_betting_enabled", "staking_logic_enabled", "roi_logic_enabled"]:
