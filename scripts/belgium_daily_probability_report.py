# -*- coding: utf-8 -*-
"""Belgian Pro League Daily Probability Report.

Same style as the existing daily probability reports (Eredivisie, La Liga, etc.).
Uses B1 historical match data and falls back to the top-5 shared model when no
Belgium-specific model exists.

Usage:
    python scripts/belgium_daily_probability_report.py
"""
...
FIXTURE_FILE  = ROOT / "data" / "upcoming_belgium_fixtures.csv"

# Primary: Belgium-specific model (if it exists); fallback: shared top-5 model
_MODEL_BELGIUM = ROOT / "outputs" / "model_comparison_belgium" / "best_model.joblib"
...
_HIST_PROCESSED = ROOT / "data" / "processed" / "belgium_clean.csv"
_HIST_RAW_2025  = ROOT / "data" / "raw" / "football_data_B1_2025.csv"
_HIST_RAW_2024  = ROOT / "data" / "raw" / "football_data_B1_2024.csv"
...
# History: prefer processed CSV; chain raw files as fallback
if _HIST_PROCESSED.exists():
    print(f"Loading Belgium history (processed): {_HIST_PROCESSED}")
    ...
    if not raw_parts:
        print("[WARN] No Belgium historical data found. "
              "Place football_data_B1_2024.csv / football_data_B1_2025.csv in data/raw/")
        sys.exit(1)
    raw = pd.concat(raw_parts, ignore_index=True)
    # football-data.org raw format columns → normalise
    col_map = {
    ...
    history = raw
    print(f"Loading Belgium history (raw B1 files): {len(history)} matches")
...
print()
print(SEP)
print(f"  BELGIAN PRO LEAGUE DAILY PROBABILITY REPORT  --  {REPORT_DATE}")
print(f"  History: {len(history)} Belgian Pro League matches")
...
for _, fix in fixtures.iterrows():
    ...
    # Control/chaos (same formula as other reports)
    ...
    print(SEP)
    print(f"  Belgian Pro League {game_time}  |  {home} vs {away}")
    print(SEP)
...
print("  Profile: belgium_balanced_goals (new league — low sample size warning active).")
print(SEP)
print()
