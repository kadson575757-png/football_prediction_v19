...
def analyze_ppg_probability_shift(rows: str | Path, output_dir: str | Path = "outputs/v291_ppg_probability_shift") -> dict[str, object]:
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    frame = pd.read_csv(rows, keep_default_na=False)
    work = frame.copy()
    shadow_map = {
        "ppg_adjusted_home_win_probability": "home_win_probability",
        "ppg_adjusted_draw_probability": "draw_probability",
        "ppg_adjusted_away_probability": "away_win_probability",
    }
    for target, fallback in shadow_map.items():
        if target not in work.columns:
            work[target] = work.get(fallback, 0)
    for column in ["base_home_win_probability", "base_draw_probability", "base_away_probability", "ppg_adjusted_home_win_probability", "ppg_adjusted_draw_probability", "ppg_adjusted_away_probability"]:
        work[column] = pd.to_numeric(work.get(column, 0), errors="coerce").fillna(0.0)
    work["home_shift"] = work["ppg_adjusted_home_win_probability"] - work["base_home_win_probability"]
    work["draw_shift"] = work["ppg_adjusted_draw_probability"] - work["base_draw_probability"]
    work["away_shift"] = work["ppg_adjusted_away_probability"] - work["base_away_probability"]
    applied = work.get("ppg_adjustment_applied", pd.Series([False] * len(work))).astype(str).str.lower().isin(["true", "1", "yes"])
    ...
    summary = {
        "v291_ppg_probability_shift_status": "READY",
        "rows_analyzed": int(len(work)),
        "ppg_adjustment_applied_count": int(applied.sum()),
        ...
        "biggest_home_ppg_upgrades": _top_rows(work, "home_shift"),
        "biggest_away_ppg_upgrades": _top_rows(work, "away_shift"),
        ...
        "hit_rate_adjusted_rows": _rate(int((hits & applied).sum()), int((decisions & applied).sum())),
        "hit_rate_unchanged_rows": _rate(int((hits & ~applied).sum()), int((decisions & ~applied).sum())),
        "automatic_betting_enabled": False,
        "staking_logic_enabled": False,
        "roi_logic_enabled": False,
    }
    rows_path = out / "v291_ppg_probability_shift_rows.csv"
    json_path = out / "v291_ppg_probability_shift_summary.json"
    md_path = out / "v291_ppg_probability_shift_report.md"
...
def _top_rows(frame: pd.DataFrame, column: str) -> list[dict[str, object]]:
    cols = ["competition", "home_team", "away_team", "match_date", column, "home_away_ppg_diff"]
...
def _markdown(summary: dict[str, object]) -> str:
    return "\n".join(
        [
            "# v2.9.1 Home/Away PPG Probability Shift",
            "",
            f"- rows_analyzed: {summary['rows_analyzed']}",
            f"- ppg_adjustment_applied_count: {summary['ppg_adjustment_applied_count']}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", default="outputs/v291_ppg_probability_shift")
    parser.add_argument("--emit-all", action="store_true")
    args = parser.parse_args(argv)
    result = analyze_ppg_probability_shift(args.rows, args.output_dir)
    for key in [
        "v291_ppg_probability_shift_status",
        "rows_analyzed",
        "ppg_adjustment_applied_count",
        "average_home_shift",
        "average_away_shift",
        "average_draw_shift",
        "hit_rate",
        "hit_rate_adjusted_rows",
        "hit_rate_unchanged_rows",
