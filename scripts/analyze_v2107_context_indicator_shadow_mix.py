...
INDICATORS = {
    "rsp": "RESULT_STREAK_PROFILE",
    "srp": "SCORING_RUN_PROFILE",
    "h2hc": "HEAD_TO_HEAD_CONTEXT_PROFILE",
    "lzp": "LEAGUE_ZONE_PRESSURE_PROFILE",
}


def analyze_context_indicator_shadow_mix(rows: str | Path, output_dir: str | Path = "outputs/v2107_context_indicator_shadow_mix") -> dict[str, object]:
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    frame = pd.read_csv(rows, keep_default_na=False)
    work = _prepare(frame)
    summary = _summary(work)
    rows_path = out / "v2107_context_indicator_shadow_mix_rows.csv"
    json_path = out / "v2107_context_indicator_shadow_mix_summary.json"
    md_path = out / "v2107_context_indicator_shadow_mix_report.md"
...
def _prepare(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for prefix in list(INDICATORS) + ["v2107_mix", "v2107_combined_mix"]:
...
def _summary(work: pd.DataFrame) -> dict[str, object]:
    ...
    v2107_rate = _hit_rate(work["v2107_mix_shadow_result"])
    combined_rate = _hit_rate(work["v2107_combined_mix_shadow_result"])
    overall = {"v2107_mix": v2107_rate, "v2107_combined_mix": combined_rate, **{INDICATORS[k]: v["shadow_top_hit_rate"] for k, v in indicator_summaries.items()}}
    best_overall_name = max(overall, key=overall.get) if overall else ""
    summary = {
        "v2107_context_indicator_shadow_mix_status": "READY",
        "rows_analyzed": int(len(work)),
        "base_top_probability_hit_rate": base_rate,
        "v2107_mix_top_hit_rate": v2107_rate,
        "v2107_mix_top_home_count": int(work["v2107_mix_shadow_top_outcome"].eq("HOME").sum()),
        "v2107_mix_top_draw_count": int(work["v2107_mix_shadow_top_outcome"].eq("DRAW").sum()),
        "v2107_mix_top_away_count": int(work["v2107_mix_shadow_top_outcome"].eq("AWAY").sum()),
        "v2107_mix_changed_top_outcome_count": int((work["v2107_mix_shadow_top_outcome"] != work["base_shadow_top_outcome"]).sum()),
        "v2107_combined_mix_top_hit_rate": combined_rate,
        "v2107_combined_mix_top_home_count": int(work["v2107_combined_mix_shadow_top_outcome"].eq("HOME").sum()),
        "v2107_combined_mix_top_draw_count": int(work["v2107_combined_mix_shadow_top_outcome"].eq("DRAW").sum()),
        "v2107_combined_mix_top_away_count": int(work["v2107_combined_mix_shadow_top_outcome"].eq("AWAY").sum()),
        "v2107_combined_mix_changed_top_outcome_count": int((work["v2107_combined_mix_shadow_top_outcome"] != work["base_shadow_top_outcome"]).sum()),
        ...
        "v2107_mix_vs_base_delta": round(v2107_rate - base_rate, 4),
        "v2107_combined_mix_vs_base_delta": round(combined_rate - base_rate, 4),
        "recommendation": _recommendation(base_rate, best_rate, v2107_rate, combined_rate),
...
def _recommendation(base: float, best: float, v2107: float, combined: float) -> str:
    if combined > base and combined >= max(best, v2107):
        return "V2107_COMBINED_MIX_PROMISING"
    if v2107 > base and v2107 >= best:
        return "CONTEXT_INDICATORS_PROMISING"
    if best > base and best > max(v2107, combined):
        return "SINGLE_INDICATOR_STRONGER_THAN_MIX"
    if max(best, v2107, combined) <= base:
...
def _markdown(summary: dict[str, object]) -> str:
    return "\n".join(["# v2.10.7 Context Indicator Shadow Mix", "", f"- rows_analyzed: {summary['rows_analyzed']}", f"- base_top_probability_hit_rate: {summary['base_top_probability_hit_rate']}", f"- v2107_mix_top_hit_rate: {summary['v2107_mix_top_hit_rate']}", f"- v2107_combined_mix_top_hit_rate: {summary['v2107_combined_mix_top_hit_rate']}", f"- recommendation: {summary['recommendation']}", "", "Safety: automatic_betting_enabled=false, staking_logic_enabled=false, roi_logic_enabled=false."])


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--rows", default="outputs/v27_prematch_evaluation/v27_prematch_evaluation_rows.csv")
    parser.add_argument("--output-dir", default="outputs/v2107_context_indicator_shadow_mix")
    parser.add_argument("--emit-all", action="store_true")
    args = parser.parse_args(argv)
    result = analyze_context_indicator_shadow_mix(args.rows, args.output_dir)
    for key in ["v2107_context_indicator_shadow_mix_status", "rows_analyzed", "base_top_probability_hit_rate", "rsp_shadow_top_hit_rate", "srp_shadow_top_hit_rate", "h2hc_shadow_top_hit_rate", "lzp_shadow_top_hit_rate", "v2107_mix_top_hit_rate", "v2107_combined_mix_top_hit_rate", "best_single_indicator_name", "best_single_indicator_hit_rate", "best_overall_shadow_name", "best_overall_shadow_hit_rate", "v2107_mix_vs_base_delta", "v2107_combined_mix_vs_base_delta", "recommendation", "automatic_betting_enabled", "staking_logic_enabled", "roi_logic_enabled"]:
