...
INDICATORS = {
    "adm": "ATTACK_DEFENSE_MATCHUP_PROFILE",
    "vsd": "VENUE_SPLIT_DELTA_PROFILE",
    "dpc": "DRAW_PRESSURE_COMPOSITE_PROFILE",
    "sca": "SHADOW_CONSENSUS_ALIGNMENT_PROFILE",
}


def analyze_matchup_interaction_shadow_mix(rows: str | Path, output_dir: str | Path = "outputs/v2109_matchup_interaction_shadow_mix") -> dict[str, object]:
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    frame = pd.read_csv(rows, keep_default_na=False)
    work = _prepare(frame)
    summary = _summary(work)
    rows_path = out / "v2109_matchup_interaction_shadow_mix_rows.csv"
    json_path = out / "v2109_matchup_interaction_shadow_mix_summary.json"
    md_path = out / "v2109_matchup_interaction_shadow_mix_report.md"
...
def _prepare(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for prefix in list(INDICATORS) + ["v2109_mix", "v2109_combined_mix"]:
...
def _summary(work: pd.DataFrame) -> dict[str, object]:
    ...
    v2109_rate = _hit_rate(work["v2109_mix_shadow_result"])
    combined_rate = _hit_rate(work["v2109_combined_mix_shadow_result"])
    overall = {"v2109_mix": v2109_rate, "v2109_combined_mix": combined_rate, **{INDICATORS[k]: v["shadow_top_hit_rate"] for k, v in indicator_summaries.items()}}
    best_overall_name = max(overall, key=overall.get) if overall else ""
    summary = {
        "v2109_matchup_interaction_shadow_mix_status": "READY",
        "rows_analyzed": int(len(work)),
        "base_top_probability_hit_rate": base_rate,
        "v2109_mix_top_hit_rate": v2109_rate,
        "v2109_mix_top_home_count": int(work["v2109_mix_shadow_top_outcome"].eq("HOME").sum()),
        "v2109_mix_top_draw_count": int(work["v2109_mix_shadow_top_outcome"].eq("DRAW").sum()),
        "v2109_mix_top_away_count": int(work["v2109_mix_shadow_top_outcome"].eq("AWAY").sum()),
        "v2109_mix_changed_top_outcome_count": int((work["v2109_mix_shadow_top_outcome"] != work["base_shadow_top_outcome"]).sum()),
        "v2109_combined_mix_top_hit_rate": combined_rate,
        "v2109_combined_mix_top_home_count": int(work["v2109_combined_mix_shadow_top_outcome"].eq("HOME").sum()),
        "v2109_combined_mix_top_draw_count": int(work["v2109_combined_mix_shadow_top_outcome"].eq("DRAW").sum()),
        "v2109_combined_mix_top_away_count": int(work["v2109_combined_mix_shadow_top_outcome"].eq("AWAY").sum()),
        "v2109_combined_mix_changed_top_outcome_count": int((work["v2109_combined_mix_shadow_top_outcome"] != work["base_shadow_top_outcome"]).sum()),
        ...
        "v2109_mix_vs_base_delta": round(v2109_rate - base_rate, 4),
        "v2109_combined_mix_vs_base_delta": round(combined_rate - base_rate, 4),
        "recommendation": _recommendation(base_rate, best_rate, v2109_rate, combined_rate),
...
def _recommendation(base: float, best: float, v2109: float, combined: float) -> str:
    if combined > base and combined >= max(best, v2109):
        return "V2109_COMBINED_MIX_PROMISING"
    if v2109 > base and v2109 >= best:
        return "MATCHUP_INTERACTION_INDICATORS_PROMISING"
    if best > base and best > max(v2109, combined):
        return "SINGLE_INDICATOR_STRONGER_THAN_MIX"
    if max(best, v2109, combined) <= base:
...
def _markdown(summary: dict[str, object]) -> str:
    return "\n".join(["# v2.10.9 Matchup Interaction Shadow Mix", "", f"- rows_analyzed: {summary['rows_analyzed']}", f"- base_top_probability_hit_rate: {summary['base_top_probability_hit_rate']}", f"- v2109_mix_top_hit_rate: {summary['v2109_mix_top_hit_rate']}", f"- v2109_combined_mix_top_hit_rate: {summary['v2109_combined_mix_top_hit_rate']}", f"- recommendation: {summary['recommendation']}", "", "Safety: automatic_betting_enabled=false, staking_logic_enabled=false, roi_logic_enabled=false."])


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--rows", default="outputs/v27_prematch_evaluation/v27_prematch_evaluation_rows.csv")
    parser.add_argument("--output-dir", default="outputs/v2109_matchup_interaction_shadow_mix")
    parser.add_argument("--emit-all", action="store_true")
    args = parser.parse_args(argv)
    result = analyze_matchup_interaction_shadow_mix(args.rows, args.output_dir)
    for key in ["v2109_matchup_interaction_shadow_mix_status", "rows_analyzed", "base_top_probability_hit_rate", "adm_shadow_top_hit_rate", "vsd_shadow_top_hit_rate", "dpc_shadow_top_hit_rate", "sca_shadow_top_hit_rate", "v2109_mix_top_hit_rate", "v2109_combined_mix_top_hit_rate", "best_single_indicator_name", "best_single_indicator_hit_rate", "best_overall_shadow_name", "best_overall_shadow_hit_rate", "v2109_mix_vs_base_delta", "v2109_combined_mix_vs_base_delta", "recommendation", "automatic_betting_enabled", "staking_logic_enabled", "roi_logic_enabled"]:
