# -*- coding: utf-8 -*-
"""2. Bundesliga Daily Probability Report.
...
MODEL_FILE = ROOT / "outputs" / "model_comparison_d2" / "best_model.joblib"
...
def control_chaos(p_h: float, p_d: float, p_a: float, h_form: dict, a_form: dict) -> tuple[float, float]:
...
print(f"Loading 2. Bundesliga history: {HISTORY_FILE}")
...
print()
print(SEP)
print(f"  2. BUNDESLIGA DAILY PROBABILITY REPORT  --  {REPORT_DATE}")
print(f"  History: {len(history)} 2. Bundesliga matches")
print(f"  Model  : 2. Bundesliga-specific" + (f" (accuracy={accuracy:.3f})" if accuracy is not None else ""))
print(SEP)
print()
...
for _, fix in fixtures.iterrows():
    ...
    print(SEP)
    print(f"  2. Bundesliga {game_time}  |  {home} vs {away}")
    print(SEP)
    ...
    print(f"\n  Control Score : {ctrl:5.1f} / 100  [{'#' * int(ctrl / 10):<10}]")
    print(f"  Chaos Score   : {chaos:5.1f} / 100  [{'#' * int(chaos / 10):<10}]")
    ...
    recommended_market = apply_league_market_profile(recommended_market, "2. Bundesliga")
    ...
    print("\n  LEAGUE PROFILE  [2. Bundesliga / diagnostic only]")
    ...
    if no_conf:
        print(f"\n  ** NO-CONFIDENCE **  Draw p={p_d*100:.0f}% or control={ctrl:.0f}/100 - outcome unclear.")
    print()
...
print(SEP)
print("  Control/Chaos scale: 0-100")
print(f"  {'Game':<34} {'1X2':>5} {'%':>4} {'Conf':>13} {'Ctrl':>6} {'Chaos':>6} {'Type':<13} {'Strength':<8}  Goals")
print(SEP2)
for r in results:
    game = f"{r['home'][:15]} vs {r['away'][:15]}"
    nc = " *NC*" if r["no_conf"] else ""
    print(
        f"  {game:<34} {r['best_1x2']:>5} {r['best_prob']*100:3.0f}% "
        f"{r['conf']:>13} {r['ctrl']:6.1f} {r['chaos']:6.1f} "
        f"{r['recommended_market']['recommended_market_type']:<13} {r['recommended_market']['recommendation_strength']:<8}  "
        f"{r['goals_pic']}{nc}"
    )
...
print(SEP)
print(f"  {'Match':<38} {'Likely':>6} {'Control':>8} {'Chaos':>7} {'Confidence':>13}  Interpretation")
print(SEP2)
for r in results:
    game = f"{r['home'][:17]} vs {r['away'][:17]}"
    print(
        f"  {game:<38} {r['best_1x2']:>6} {r['ctrl']:8.1f} {r['chaos']:7.1f} "
...
cleanest_profile = max(results, key=lambda r: (r["ctrl"] - r["chaos"], r["ctrl"]))
most_dangerous_profile = max(results, key=lambda r: (r["chaos"] - r["ctrl"], r["chaos"]))

print(f"\n  CONTROL / CHAOS EXTREMES:")
print(f"    Highest control_score: {highest_control['home']} vs {highest_control['away']}  control={highest_control['ctrl']:.1f}/100  chaos={highest_control['chaos']:.1f}/100")
print(f"    Lowest control_score : {lowest_control['home']} vs {lowest_control['away']}  control={lowest_control['ctrl']:.1f}/100  chaos={lowest_control['chaos']:.1f}/100")
print(f"    Highest chaos_score  : {highest_chaos['home']} vs {highest_chaos['away']}  chaos={highest_chaos['chaos']:.1f}/100  control={highest_chaos['ctrl']:.1f}/100")
print(f"    Lowest chaos_score   : {lowest_chaos['home']} vs {lowest_chaos['away']}  chaos={lowest_chaos['chaos']:.1f}/100  control={lowest_chaos['ctrl']:.1f}/100")
print(f"    Cleanest profile     : {cleanest_profile['home']} vs {cleanest_profile['away']}  {control_chaos_interpretation(cleanest_profile['ctrl'], cleanest_profile['chaos'])}")
