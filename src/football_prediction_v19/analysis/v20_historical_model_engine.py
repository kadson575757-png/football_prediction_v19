...
def run_v20_model_engine(features: dict[str, object], output_dir: str | Path) -> dict[str, object]:
    ...
    if features.get("leakage_status") != "CLEAN":
    ...
    elif features.get("table_available") is False:
        probs = {"HOME": 0, "DRAW": 0, "AWAY": 0}; status = "MODEL_BLOCKED"; confidence = 0.0; risk = 1.0
    else:
        ...
        quality = float(features.get("data_quality_score", 0))
        missing_inputs = []
        if features.get("xg_available") is False:
        ...
        confidence = round(min(0.9, 0.35 + quality * 0.45), 3); risk = round(1 - confidence, 3)
        status = "MODEL_READY" if quality >= 0.9 and not missing_inputs else "MODEL_PARTIAL"
    missing_inputs = [] if status == "MODEL_BLOCKED" else [name for name in ["xg", "odds"] if features.get(f"{name}_available") is False]
    result = {"model_status": status, "home_win_probability": probs["HOME"], "draw_probability": probs["DRAW"], "away_win_probability": probs["AWAY"], "over_2_5_probability": float(features.get("over25_implied_probability_asof", 0) or 0), "model_confidence": confidence, "model_risk_score": risk, "model_data_quality_score": features.get("data_quality_score", 0), "warnings": ["lineups/injuries missing are risk warnings, not hard blockers"] if not features.get("lineups_available", True) else [], "missing_inputs": missing_inputs, "automatic_betting_enabled": False, "staking_logic_enabled": False, "roi_logic_enabled": False}
