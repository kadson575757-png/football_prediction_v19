...
def run_tip_decision_engine(model: dict[str, object], asof_status: str, features: dict[str, object], output_dir: str | Path) -> dict[str, object]:
    ...
    elif float(features.get("data_quality_score", 0)) < 0.5:
    ...
    result = {"decision_class": decision, "primary_tip": tip, "secondary_tip": "", "confidence": model.get("model_confidence", 0), "risk_level": "HIGH" if float(model.get("model_risk_score", 1)) > 0.5 else "MEDIUM", "why": "As-of table, xG and odds support this preview decision.", "why_not_stronger": "No stake, ROI or automatic betting; confidence depends on source coverage.", "missing_data": "", "no_bet_reasons": "Data quality or leakage gate blocked." if tip == "NO_BET" else "", "automatic_betting_enabled": False, "staking_logic_enabled": False, "roi_logic_enabled": False}
