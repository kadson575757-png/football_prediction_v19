...
def run_tip_decision_engine(model: dict[str, object], asof_status: str, features: dict[str, object], output_dir: str | Path) -> dict[str, object]:
    ...
    elif model.get("model_status") == "MODEL_PARTIAL" and (not features.get("xg_available") or not features.get("odds_available")):
    ...
    result = {"decision_class": decision, "primary_tip": tip, "secondary_tip": "", "confidence": model.get("model_confidence", 0), "risk_level": "HIGH" if float(model.get("model_risk_score", 1)) > 0.5 else "MEDIUM", "why": "As-of table, xG and odds support this preview decision." if tip != "NO_BET" else "Coverage, confidence or leakage policy prevents a production recommendation.", "why_not_stronger": "No stake, ROI or automatic betting; confidence depends on source coverage.", "missing_data": missing, "no_bet_reasons": "Data quality, missing source coverage or leakage gate blocked." if tip == "NO_BET" else "", "automatic_betting_enabled": False, "staking_logic_enabled": False, "roi_logic_enabled": False}
