...
def build_goals_against_indicator(
    competition: str,
    season: str,
    home_team: str,
    away_team: str,
    ...
    home_totals = _team_goals_against_totals(work, home_team)
    away_totals = _team_goals_against_totals(work, away_team)
    home_n = int(home_totals["matches"])
    away_n = int(away_totals["matches"])
    home_ga = int(home_totals["goals_against"])
    away_ga = int(away_totals["goals_against"])
    home_gapm = round(float(home_ga / home_n), 4) if home_n else 0.0
    away_gapm = round(float(away_ga / away_n), 4) if away_n else 0.0
    quality = "FULL" if home_n >= 8 and away_n >= 8 else ("PARTIAL" if home_n >= 3 and away_n >= 3 else "LOW")
    reason = "Goals Against defensive advantage built from matches before match_date" if quality != "LOW" else "Not enough prior matches for both teams"
    return {
        "home_matches_before_match": home_n,
        "away_matches_before_match": away_n,
        "home_goals_against_before_match": home_ga,
        "away_goals_against_before_match": away_ga,
        "home_goals_against_per_match_before_match": home_gapm,
        "away_goals_against_per_match_before_match": away_gapm,
        "goals_against_advantage_diff": round(away_gapm - home_gapm, 4),
        "goals_against_indicator_quality": quality,
        "goals_against_indicator_reason": reason,
    }
...
def _load_match_rows(competition: str, season: str, home_team: str, away_team: str, match_date: str, *, cache_only: bool, enable_network: bool) -> pd.DataFrame:
    out = Path("outputs/v297_goals_against") / _slug(f"{competition}_{season}_{home_team}_vs_{away_team}")
...
def _team_goals_against_totals(frame: pd.DataFrame, team: str) -> dict[str, int]:
    ...
    return {"matches": matches, "goals_against": goals_against}
...
def _empty(quality: str, reason: str) -> dict[str, object]:
    return {
        ...
        "home_goals_against_per_match_before_match": 0.0,
        "away_goals_against_per_match_before_match": 0.0,
        "goals_against_advantage_diff": 0.0,
        "goals_against_indicator_quality": quality,
        "goals_against_indicator_reason": reason,
    }
