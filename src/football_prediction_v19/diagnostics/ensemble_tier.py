...
def apply_ensemble_override(
    ...
    dict with keys:
      ...
      "ensemble_agreement" — the agreement value as passed in
      "ensemble_note"      — "CONSENSUS" | "SPLIT" | "DISAGREEMENT"
    """
    market_tier = str(existing_tier)
    reason: str = ""
    flags: list[str] = []

    # -- ensemble_note based purely on agreement value -----------------------
    if agreement == 1.0:
        ensemble_note = "CONSENSUS"
    elif agreement == 0.5:
        ensemble_note = "SPLIT"
    else:
        ensemble_note = "DISAGREEMENT"
